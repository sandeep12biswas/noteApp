...
| Electron shell (Linux) | ✅ Sidecar start (`SidecarSupervisor`, crash-restart with backoff per DESIGN.md §10), the `electron-trpc` router (thin pass-through to the sidecar per method), and the preload bridge (`window.electronTRPC`) are wired and unit-tested (11 tests) — verified end-to-end by driving the real compiled sidecar binary over the actual protocol from Node. ✅ `electron-builder.yml` packages the frontend + release sidecar binary as `extraResources`; verified with a real `--dir` build (correct `resources/frontend`, `resources/bin/flownote-electron`, executable named `flownote`) — this also caught and fixed a real path-resolution bug in `main.ts`. ⬜ Not yet launched as a real window (no working display server in this environment — Electron hangs at native Chromium startup before reaching any FlowNote code, both in dev and packaged form). |
