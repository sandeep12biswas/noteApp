...
| Electron shell (Linux) | ✅ Sidecar start (`SidecarSupervisor`, crash-restart with backoff per DESIGN.md §10), the `electron-trpc` router (thin pass-through to the sidecar per method), and the preload bridge (`window.electronTRPC`) are wired and unit-tested (11 tests) — verified end-to-end by driving the real compiled sidecar binary over the actual protocol from Node. ⬜ `electron-builder` packaging config not done. ⬜ Not yet launched as a real window (no display server in this environment). |
