...
function render(rows: number, cols: number, data: Record<string, string>) {
  root.innerHTML = ''
...
let blockId = ''
block.onInit((id: string, attrs: Record<string, unknown>) => {
  blockId = id
  const rows = typeof attrs.rows === 'number' ? attrs.rows : 5
  const cols = typeof attrs.cols === 'number' ? attrs.cols : 4
  const data = (attrs.data as Record<string, string> | undefined) ?? {}
